...
export class AdminPanel {
  ...
  static adminPanel: HTMLElement = <HTMLElement>(
    document.getElementById("adminPanel")
  );

  static commitEditBtn: HTMLElement = <HTMLElement>(
    document.getElementById("commitEditBtn")
  );

  static commitDelBtn: HTMLElement = <HTMLElement>(
    document.getElementById("commitDelBtn")
  );
  ...
  static setUpCommitBtns() {
    AddTable.commitBtn.onclick = () => {
      ...
      if (addedContent.length <= 0) alert("No added content");
      else {
        let ok: boolean = confirm(
          `Are you sure you want to add those content?`
        );
        if (ok) {
          this.dao
    ...
    this.commitDelBtn.onclick = () => {
      if (this.dao.deletedIDs.length <= 0) alert("No deleted IDs found!");
      else {
        let ok: boolean = confirm(
          `Are you sure you want to delete these ids: ${Helper.ObjArrToString(
            this.dao.deletedIDs
          )}?`
        );
        if (ok) {
  ...
  static superviseEdit(row: HTMLElement) {
    ...
    if (
      ...
      for (let i = 0; i < this.dao.getHeaders().length; i++) {
        ...
        if (rowEditedContent?.hasOwnProperty(header)) {
  ...
  static addEditedContent(newContent: { [key: string]: any }, dao: DAO) {
