    ...
    class DataTable {
            ...
            for (let i = 0; i < tbody.childElementCount; i++) {
                ...
                edit.onclick = () => {
                    ...
                    if (!AdminPanel.editingIDs.includes(parseInt(id))) {
                        ...
                        if (AdminPanel.getSavedIDs().includes(parseInt(id))) {
                            AdminPanel.editedContent = AdminPanel.editedContent.filter((content) => content.id != id);
                        }
                        AdminPanel.refresh();
                ...
                save.onclick = () => {
                    ...
                    if (index >= 0) {
                        ...
                        AdminPanel.addEditedContent(Helper.keepDiffereces(this.datas.find((data) => {
                            if (data["id"] == id)
                                return data;
                        }), newContent));
                        AdminPanel.refresh();
        ...
        queryFunctions() {
            ...
            Array.from(headerRow.children).forEach((child) => {
                ...
                searchDiv.innerHTML = this.headers[Array.from(headerRow.children).indexOf(child)];
        ...
        displayContent(datas, start, end) {
            let tbody = DataTable.table.children[1];
            if (tbody == null) {
        ...
        displayContentWithPagination() {
            ...
            let num_of_paginations = Math.ceil(this.datas.length / recordsPerPage);
            ...
            for (let i = 0; i < num_of_paginations; i++) {
                ...
                pagination.children[i].onclick = () => {
                    let end = (i + 1) * recordsPerPage - 1 > this.datas.length - 1
                        ? this.datas.length - 1
        ...
        responsiveHeaders() {
            ...
            for (let i = 0; i < this.headers.length; i++) {
                rule += `.dataTable td:nth-of-type(${i + 1}):before { content: "${this.headers[i]}"; }\n`;
            }
            if (Helper.isAdmin())
                rule += `.dataTable td:nth-of-type(${this.headers.length + 1}):before { content: "Operations"; }\n`;
            rule += "}";
            styleSheet.insertRule(rule);
        }
        fillOneRow(content, row, type = "td") {
